...
public class SetMaxCommand extends BaseCommand {
    ...
    public SetMaxCommand(GameSettings settings) {
        super("setmax", "Set the maximum time between swaps", "/setmax <seconds>", new ArrayList<>());
    ...
    public void run(@NotNull CommandSender sender, @NotNull String alias, @NotNull List<String> args) {
        if (args.size() == 0) {
            sender.sendMessage(ChatColor.RED + "You must enter the maximum number of seconds between swaps");
            return;
        }
        ...
        if (seconds >= 10 && seconds <= 3600) {
            ...
            sender.sendMessage(ChatColor.GREEN + "Successfully changed the maximum teleport time to " + ChatColor.LIGHT_PURPLE + seconds + " seconds");
        } else {
            sender.sendMessage(ChatColor.RED + "You must enter a valid number between 10 and 3600");
        }
    }
